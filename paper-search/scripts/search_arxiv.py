...
type Paper = dict[str, object]
type ConfigDict = dict[str, object]
type DomainConfig = dict[str, object]
...
def as_paper_list(value: object) -> list[Paper]:
    if not isinstance(value, list):
        return []
    items = cast(list[object], value)
...
def paper_score(paper: Paper) -> float:
    scores = as_dict(paper.get('scores'))
...
has_requests = requests_lib is not None
...
def load_research_config(config_path: str) -> ConfigDict:
    """
    从 YAML 文件加载研究兴趣配置
    ...
    try:
        ...
        config = as_dict(loaded)
        global s2_api_key
        s2_api_key = as_str(config.get('semantic_scholar_api_key')) or None
        return config
...
def calculate_date_windows(target_date: datetime | None = None) -> tuple[datetime, datetime, datetime, datetime]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.info("[arXiv] Retrying in %d seconds...", delay)
                time.sleep(delay)
            else:
    ...
    for attempt in range(max_retries):
        try:
            if requests_lib is not None:
                ...
                data = cast(Paper, response.json())
            else:
                # 使用 urllib
                query_string = urllib.parse.urlencode(params)
                url = f"{SEMANTIC_SCHOLAR_API_URL}?{query_string}"
                req = urllib.request.Request(url, headers=headers)
                response_handle = cast(UrlOpenResponse, urllib.request.urlopen(req, timeout=15))
                with response_handle as response:
                    response_bytes = response.read()
                    data = cast(Paper, json.loads(response_bytes.decode('utf-8')))
            
            papers = cast(list[Paper], data.get("data", []))
            if not papers:
            ...
            for p in papers:
                ...
                if p.get('authors') and not p.get('affiliations'):
                    ...
                    for a in cast(list[Paper], p['authors']):
                        for affil in cast(list[object], a.get('affiliations') or []):
                            affil_dict = as_dict(affil)
                            name = as_str(affil_dict.get('name')) if affil_dict else str(affil)
            ...
            # 按高影响力引用数倒序排列
            sorted_papers: list[Paper] = sorted(
                valid_papers,
                key=lambda x: as_int(x.get("influentialCitationCount"), 0),
                reverse=True
            )
            ...
            is_rate_limit = False
            if requests_lib is not None and hasattr(e, 'response') and getattr(e, 'response') is not None:
                response_obj = cast(object, getattr(e, 'response'))
                status_code = as_int(getattr(response_obj, 'status_code', 0), 0)
            ...
            if attempt < max_retries - 1:
                # 对于 429 错误，使用更长的等待时间
                if is_rate_limit:
                    logger.warning("[S2] Rate limit hit. Waiting %d seconds...", S2_RATE_LIMIT_WAIT)
                    time.sleep(S2_RATE_LIMIT_WAIT)
                else:
                    delay = retry_delay(attempt)
                    logger.info("[S2] Retrying in %d seconds...", delay)
                    time.sleep(delay)
            else:
...
def search_hot_papers_from_categories(
    ...
    Returns:
        合并后的高影响力论文列表
    """
    all_hot_papers: list[Paper] = []
    seen_arxiv_ids: set[str] = set()

    # 从配置中提取用户自定义的搜索关键词（更精准）
    user_queries: list[str] = []
    if config:
        domains = as_dict(config.get('research_domains'))
        for domain_name, domain_config in domains.items():
            _ = domain_name
            domain_config_dict = as_dict(domain_config)
            keywords = as_str_list(domain_config_dict.get('keywords'))
            # 取每个域的前3个关键词组合为查询
            if keywords:
    ...
    for query in unique_queries:
        ...
        for p in papers:
            ...
            external_ids = as_dict(p.get("externalIds"))
            arxiv_id = as_str(external_ids.get("ArXiv")) or None
    ...
    # 最终按影响力引用数排序
    all_hot_papers.sort(key=lambda x: as_int(x.get("influentialCitationCount"), 0), reverse=True)
    
    return all_hot_papers


def parse_arxiv_xml(xml_content: str) -> list[Paper]:
    """
    解析 arXiv XML 结果
    ...
    try:
        ...
        for entry in root.findall('atom:entry', ARXIV_NS):
            ...
            if id_elem is not None:
                ...
                paper_id = as_str(paper.get('id'))
                match = re.search(r'arXiv:(\d+\.\d+)', paper_id)
                if match:
            ...
            title_text = as_str(title_elem.text) if title_elem is not None else ""
            if title_text:
                paper['title'] = title_text.strip()
            
            # 提取摘要
            summary_elem = entry.find('atom:summary', ARXIV_NS)
            summary_text = as_str(summary_elem.text) if summary_elem is not None else ""
            if summary_text:
                paper['summary'] = summary_text.strip()
            ...
            for author in entry.findall('atom:author', ARXIV_NS):
                ...
                name_text = as_str(name_elem.text) if name_elem is not None else ""
                if name_text:
            ...
            if published_elem is not None:
                published_text = as_str(published_elem.text)
                paper['published'] = published_text
                # 解析日期
                try:
...
def calculate_relevance_score(
    ...
    """
    title = as_str(paper.get('title')).lower()
    summary = as_str(paper.get('summary') or paper.get('abstract')).lower()
    categories = set(as_str_list(paper.get('categories')))
    ...
    for domain_name, domain_config in domains.items():
        score = 0.0
        domain_config_dict = as_dict(domain_config)
        domain_matched_keywords: list[str] = []
        ...
        # 类别匹配
        domain_categories = as_str_list(domain_config_dict.get('arxiv_categories'))
...
def filter_and_score_papers(
    ...
    """
    _ = target_date
    domains = cast(dict[str, DomainConfig], as_dict(config.get('research_domains')))
    excluded_keywords = as_str_list(config.get('excluded_keywords'))
    ...
    for paper in papers:
        ...
        if 'published_date' in paper:
            recency = calculate_recency_score(as_datetime(paper.get('published_date')))
        else:
            # 对于 Semantic Scholar 的论文，使用 publicationDate
            pub_date_str = as_str(paper.get('publicationDate'))
            if pub_date_str:
        ...
        if is_hot_paper_batch:
            ...
            inf_cit = as_int(paper.get('influentialCitationCount'), 0)
            popularity = min(
            ...
            # 最近7天的新论文可能有更高的"潜在热度"
            pub = as_datetime(paper.get('published_date'))
            if pub is not None:
        ...
        # 计算质量
        summary = as_str(paper.get('summary') or paper.get('abstract'))
    ...
    # 按推荐评分排序
    scored_papers.sort(key=paper_score, reverse=True)
...
def main() -> int:
    ...
    _ = parser.add_argument('--config', type=str,
                            default=default_config or None,
                            help='Path to research interests config file (or set OBSIDIAN_VAULT_PATH env var)')
    _ = parser.add_argument('--output', type=str, default='arxiv_filtered.json',
                            help='Output JSON file path')
    _ = parser.add_argument('--max-results', type=int, default=200,
    ...
    _ = parser.add_argument('--target-date', type=str, default=None,
                            help='Target date (YYYY-MM-DD) for filtering')
    _ = parser.add_argument('--categories', type=str,
                            default='cs.AI,cs.LG,cs.CL,cs.CV,cs.MM,cs.MA,cs.RO',
                            help='Comma-separated list of arXiv categories')
    _ = parser.add_argument('--skip-hot-papers', action='store_true',
    ...
    # 按推荐评分排序
    all_scored_papers.sort(key=paper_score, reverse=True)
    ...
    for p in all_scored_papers:
        arxiv_id = as_str(p.get('arxiv_id') or p.get('arxivId'))
        if arxiv_id:
    ...
    for paper in top_papers:
        paper['note_filename'] = title_to_note_filename(as_str(paper.get('title')))
    ...
    for i, p in enumerate(top_papers, 1):
        ...
        logger.info("  %d. %s... (Score: %s)%s", i, as_str(p.get('title'), 'N/A')[:60], paper_score(p), hot_marker)
