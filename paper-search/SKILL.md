---
name: paper-search
description: 论文搜索推荐 - 搜索最新论文并生成每日推荐笔记 / Paper search and daily recommendations
---
...
所有论文按评分从高到低排列，带排名编号，使用统一的丰富格式。**不自动触发 deep-analyze-paper**，由用户决定深入分析哪篇。
...
**研究问题**：[论文要解决什么问题，2-3句话]

**方法概要**：[核心方法/技术路线，2-3句话，提及关键术语]
...
**关键结果**：[从摘要中提取的最重要数字和结论]

**与我的研究相关性**：[为什么推荐这篇，匹配了哪些关键词，对我的研究方向有什么参考价值]
...
```markdown
### #1 [[paper_note_filename|Paper Title]]
- **Authors**: [author list]
- **Affiliation**: [affiliation or "Not specified"]
- **Links**: [arXiv](link) | [PDF](link)
- **Recommendation Score**: [X.XX] / 10
- **Matched Domain**: [matched domain]
- **Notes**: [[existing_note_path]] or —

**One-line Summary**: [one-line summary of core contribution]

**Research Problem**: [what problem does this paper address, 2-3 sentences]

**Method Overview**: [core method/approach, 2-3 sentences with key terms]
...
**Key Results**: [most important numbers and conclusions from abstract]

**Relevance to My Research**: [why recommended, which keywords matched, reference value]
...
- 每篇论文带排名编号（#1, #2, ...）
- 所有论文使用相同的丰富格式，不区分"前几篇"和"其余"
- 如果论文已有笔记，在"笔记"字段引用已有路径

#### 4.2.3 用户主导的深度分析

**不自动触发 deep-analyze-paper**。推荐笔记生成后，在末尾添加操作提示：

```markdown
## 下一步

看完推荐后，告诉我你想深入分析哪篇论文，我会用 `/deep-analyze-paper` 进行渐进式详细分析。

示例：「帮我详细分析 #1 HUydra」或「分析 2603.23041」
```

当用户选择后，再调用 `/deep-analyze-paper` 对指定论文执行渐进式分析（extract-paper-images → scaffold → split → 逐节分析）。
...
- **前3篇特殊处理**：
  ...
  - 自动提取第一张图片并插入
  - 自动调用 `deep-analyze-paper` 生成详细报告
...
- **内容**：推荐列表
  - 开头包含"今日概览"
  - 所有论文统一丰富格式（带排名、推荐评分、研究问题、方法概要、与我研究的相关性）
  - **不自动触发 deep-analyze-paper**
  - 末尾提示用户选择想深入分析的论文
- **图片处理**：推荐笔记中不插入图片（图片在 deep-analyze-paper 阶段生成）
- **深度分析**：用户看完推荐后自行选择，再调用 `/deep-analyze-paper`
- **适用**：用户每天手动触发
- **笔记引用**：如果论文已有笔记，简略写并引用；如果分析需要引用历史笔记，也直接引用

## deep-analyze-paper (深度分析skill)
- **目的**：用户主动查看单篇论文，深度研究
- **适用场景**：用户自己还想要看，但AI没有整理到的论文
- **内容**：详细的论文深度分析笔记
  - 包含所有核心信息：研究问题、方法概述、方法架构、关键创新、实验结果、深度分析、相关论文对比等
  - **图文并茂**：论文中的所有图片都要用上（核心架构图、方法图、实验结果图等）
- **适用**：用户主动调用 `/deep-analyze-paper [论文ID]` 或论文标题
...
5. **生成推荐笔记（包含关键词链接）**
   ...
   - **按评分排序**：所有论文按推荐评分从高到低排列，带排名编号（#1, #2, ...）
   - **所有论文统一丰富格式**：每篇都包含推荐评分、研究问题、方法概要、核心贡献、关键结果、与我研究的相关性
   - **不自动触发 deep-analyze-paper / extract-paper-images**
   - **末尾添加操作提示**：告知用户可以选择任意论文进行深度分析
   - **关键词自动链接**（重要！）：
     - 在生成笔记后，扫描文本中的关键词
...
6. **等待用户选择**（流程结束）
   - 用户看完推荐笔记后，告知想深入分析哪篇
   - 用户选择后，再调用 `/deep-analyze-paper [论文ID]` 执行渐进式详细分析
     - 生成详细的论文分析报告
     - 在推荐笔记中添加图片和详细报告链接
...
- `deep-analyze-paper` skill（用于生成详细报告）
...
2. **提取笔记信息**：
   ...
   - tags 字段
   - **跳过 `images/index.md` 等图片索引文件**，只索引真正的论文笔记
