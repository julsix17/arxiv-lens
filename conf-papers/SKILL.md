...
本 skill 使用独立配置文件 `conf-papers.yaml`（位于 skill 目录下），与 paper-search 的 `research_interests.yaml` 完全独立：
...
复用 `paper-search` 的扫描脚本：

```bash
cd "$SKILL_DIR/../paper-search"
...
**评分说明**（与 paper-search 的区别：无新近性维度，因为年份由用户指定）：
...
- **论文分类（domain）命名规则**：domain 名称必须与 `deep-analyze-paper` 实际创建的目录名完全一致，**不得截断**
  - 正确：`[[20_Research/Papers/Foundation Models & LLM/...]]`
  - 错误：`[[20_Research/Papers/Foundation]]`（截断了 "Models & LLM" 部分）
- 详细报告由 `deep-analyze-paper` 自动生成，包含完整的论文分析
...
- **论文增加会议和引用字段**：区别于 paper-search 的 arXiv 论文
- **前 3 篇特殊处理**：
...
- **自动关键词链接**：复用 paper-search 的 link_keywords.py
...
- `paper-search` skill（复用 scan_existing_notes.py, link_keywords.py, search_arxiv.py 的评分函数）
- `extract-paper-images` skill（提取论文图片，仅限有 arXiv ID 的论文）
- `deep-analyze-paper` skill（生成详细报告，仅限有 arXiv ID 的论文）
